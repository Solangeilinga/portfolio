    ...
    return (
        <section className="min-h-screen py-12 md:py-20 w-full relative overflow-hidden">
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-40 right-20 w-40 h-40 bg-violet-500/10 rounded-full blur-3xl animate-pulse" />
            </div>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-12 md:mb-16">
                    ...
                    <div className="w-24 h-1 bg-gradient-to-r from-indigo-500 via-violet-500 to-fuchsia-500 mx-auto rounded-full"></div>
                </div>

                <div className="relative">
                    {/* Timeline line */}
                    <div className="absolute left-4 sm:left-6 md:left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-indigo-500 via-violet-500 to-fuchsia-500 hidden sm:block"></div>

                    <div className="space-y-8 sm:space-y-12">
                        {experiences.map((exp, index) => (
                            <div key={exp.id} className="relative group animate-fadeInUp" style={{ animationDelay: `${index * 0.2}s` }}>
                                {/* Timeline dot */}
                                <div className="absolute left-3 sm:left-5 md:left-6 top-6 sm:top-8 w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 bg-gradient-to-br from-indigo-500 to-fuchsia-500 rounded-full border-2 sm:border-4 border-slate-900 hidden sm:block group-hover:scale-125 transition-transform duration-300"></div>

                                {/* Card */}
                                <div className="sm:ml-12 md:ml-20 bg-white/10 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-white/20 hover:border-indigo-400/50 transition-all duration-300 hover:shadow-xl sm:hover:shadow-2xl hover:shadow-indigo-500/20 hover:-translate-y-1">
                                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4 mb-4">
                                        ...
                                        <div className="flex items-center gap-2 px-3 py-1 sm:px-4 sm:py-2 bg-indigo-500/20 rounded-full border border-indigo-400/30 self-start sm:self-auto">
                                            ...
                                            <span className="text-indigo-200 text-xs sm:text-sm font-medium whitespace-nowrap">
                                                {exp.period}
                                            </span>
                                        </div>
                                    </div>
